...
class MaoweiGSPS_Dynamic_Seq_Classifier_Humaneva():
    def __init__(self, t_his=25, t_pred=100, similar_cnt=10, dynamic_sub_len=5000, batch_size=8,
        ...
        self.actions_5 = ['Box', 'Gestures', 'Jog', 'ThrowCatch', 'Walking']
        ...
        if mode == "train" and similar_cnt > 0:
            self.data_similar_idx = np.load(similar_idx_path, allow_pickle=True)['data_multimodal'].item()
    ...
    def sample(self):
        ...
        action_idx_5 = self.actions_5.index(action.split(" ")[0])
        ...
        if self.mode == "train" and self.similar_cnt > 0 and (subject in self.data_similar_idx.keys()):
            ...
            data_similar = data_similar[:, :, 1:, :].reshape(1, self.similar_cnt, self.t_total, -1).transpose(0, 1, 3,
                                                                                                              2)  # 1, 10, 48, 125
            return data, action_idx_5, data_similar
        else:
            data = data[:, 1:, :].reshape(1, self.t_total, -1).transpose(0, 2, 1)  # 1, 48, 125
            return data, action_idx_5
        ...
        for i in range(self.dynamic_sub_len // self.batch_size):
            ...
            for i in range(self.batch_size):
                ...
                if self.mode == "train" and self.similar_cnt > 0:
                    ...
                    act_idx_5.append(sample_i[1])
                    sample_similar.append(sample_i[2])
                else:
                    sample.append(sample_i[0])
                    act_idx_5.append(sample_i[1])
            ...
            if self.mode == "train" and self.similar_cnt > 0:
                ...
                yield sample, act_idx_5, sample_similar
            else:
                yield sample, act_idx_5
    ...
    def onebyone_generator(self):
        ...
        for sub in all_subs:
            ...
            if self.is_debug:
                all_acts = [all_acts[0]]
            for act in all_acts:
                action_idx_5 = self.actions_5.index(act.split(" ")[0])
...
if __name__ == '__main__':
    ds = MaoweiGSPS_Dynamic_Seq_Classifier_Humaneva(data_path=r"G:\second_model_report_data\writing_paper2022\rebuttal\codes\other_evaluation_metrics\gsps_dataset")
    pass