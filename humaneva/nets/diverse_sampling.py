...
class NFDiagSampler(nn.Module):
    def __init__(self, nx, ny, nk):
        super(NFDiagSampler, self).__init__()
        self.nx = nx
        self.ny = ny
        self.nk = nk
    ...
    def forward(self, bases):
        ...
        batch_size, seq_len, feature_dim = bases.shape
        z = torch.randn((batch_size, self.nk, self.ny), device=bases.device)
        z, _, _ = torch.svd(z, some=True)

        samples = torch.matmul(z, bases)
        samples = samples.view(batch_size * self.nk, feature_dim)
        return samples


class DiverseSampling(Module):
    def __init__(self, node_n=42, hidden_dim=256, base_dim = 84, z_dim=64, dct_n=30, base_num_p1=30, dropout_rate=0):
        ...
        self.condition_enc = Sequential(
            GraphConvBlock(in_len=dct_n, out_len=hidden_dim, in_node_n=node_n, out_node_n=node_n, dropout_rate=dropout_rate, bias=True, residual=False),
