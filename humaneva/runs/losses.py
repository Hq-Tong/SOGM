...
from ..datas import humaneva_valid_angle_check_torch
...
def loss_diversity_v2_likedlow_divide(outputs, seperate_head, scale=50): # b, 50, 42, 60
    b, h, vc, t = outputs.shape
    all_div = []
    for oidx in range(h // seperate_head):
        all_div.append(loss_diversity_v2_likedlow(outputs[:, oidx * seperate_head:(oidx + 1) * seperate_head, :, :], scale=scale))  # [10, 48, 100], [1, 48, 100]
        for ojdx in range(oidx + 1, h // seperate_head):
            all_div.append(loss_diversity_two_part_v2_likedlow(outputs[:, oidx * seperate_head:(oidx + 1) * seperate_head, :, :], outputs[:, ojdx * seperate_head:(ojdx + 1) * seperate_head, :, :],
                                                                    scale=scale))  # [10, 48, 100], [1, 48, 100]
    all_div = torch.cat(all_div, dim=-1).mean(dim=-1).mean()
    return all_div
