...
from ..datas import MaoweiGSPS_Dynamic_Seq_H36m, MaoweiGSPS_Dynamic_Seq_H36m_ExtandDataset_T1, draw_multi_seqs_2d, get_dct_matrix, dct_transform_torch, reverse_dct_torch
...
class RunCVAE():
    def __init__(self, exp_name="h36m_t1", device="cuda:0", num_works=0, is_debug=False, args=None):
    ...
    def train(self, epoch, draw=False):
        ...
        for i, datas in enumerate(dg):
            ...
            with torch.no_grad():
                ...
                padded_inputs_dct = padded_inputs_dct.view(b, -1, self.cfg.dct_n)  # # b, 16, 3*10

                padded_gts_dct = dct_transform_torch(datas, self.dct_m, dct_n=self.cfg.dct_n)  # b, 48, 10
                padded_gts_dct = padded_gts_dct.view(b, -1, self.cfg.dct_n)  # # b, 16, 3*10
    ...
    def eval(self, epoch=-1, draw=False):
        ...
        for i, datas in enumerate(dg):
            ...
            with torch.no_grad():
                ...
                out_dct = out_dct.reshape([b*self.cfg.nk, self.cfg.node_n, -1])  # b*50, 48, 10
